...
**推荐方式：使用 Skill 自动化安装（Claude Code 用户）**

如果你使用 [Claude Code](https://claude.ai/code)，这是最简单的安装方式：

1. **下载 Skill 文件**
   ```bash
   curl -O https://raw.githubusercontent.com/rockcingma/SCRCPY-macUI/main/install-scrcpy-mac-ui.skill
   ```
   
   或直接下载：[install-scrcpy-mac-ui.skill](install-scrcpy-mac-ui.skill)

2. **安装 Skill**
   - 将 `.skill` 文件拖拽到 Claude Code 窗口
   - 或手动复制：`cp install-scrcpy-mac-ui.skill ~/.claude/skills/`

3. **运行 Skill**
   在 Claude Code 中输入：
   ```
   /install-scrcpy-mac-ui
   ```
   
   Skill 会自动引导你完成：
   - ✅ 依赖检查（adb、scrcpy、Rust、Bun）
   - ✅ 项目克隆和构建
   - ✅ 打包分发
   - ✅ 安装指引
...
**Recommended: Automated Installation with Skill (Claude Code Users)**

If you use [Claude Code](https://claude.ai/code), this is the easiest way:
...
1. **Download Skill File**
   ...
   Or download directly: [install-scrcpy-mac-ui.skill](install-scrcpy-mac-ui.skill)

2. **Install Skill**
   - Drag `.skill` file into Claude Code window
   - Or copy manually: `cp install-scrcpy-mac-ui.skill ~/.claude/skills/`
...
3. **Run Skill**
   ...
   - ✅ Dependency checks (adb, scrcpy, Rust, Bun)
   - ✅ Project cloning and building
   - ✅ Packaging for distribution
   - ✅ Installation guidance

**Option A: Download Pre-built Package (Direct Users)**
